<?php
        /*if(mb_strlen($name, $encoding="utf8") < 5 || mb_strlen($name, $encoding="utf8") > 50){
            echo "Недопустимая длина имени";

        } else if(mb_strlen($Sur)<3 || mb_strlen($Sur)>90){
    ...
    $mysql = new mysqli('localhost', 'root', '1111', 'tests');
    $mysql->query("INSERT INTO users (name, surname, email, password, city) VALUES('$name', '$Sur', '$email', '$password', '$city')");
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        $sql = "SELECT id FROM userdb WHERE username = ?";
