...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	} elseif(empty(trim($_POST["Sur"]))){
		$surname_err = "Please enter a surname.";
    } elseif(!preg_match('/^[a-zA-Z0-9_]+$/u', trim($_POST["name"]))){
        $username_err = "Name can only contain letters, numbers, and underscores.";
	} elseif(!preg_match('/^[a-zA-Z0-9_]+$/u', trim($_POST["Sur"]))){
...
<body>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
			...
			<div class="form-group">
                <label>Surname</label>
                <input type="text" name="Sur" class="form-control <?php echo (!empty($surname_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $surname; ?>">
