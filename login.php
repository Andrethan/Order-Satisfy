...
$name_err = $surname_err = $password_err = $login_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($surname_err) && empty($password_err)){
        // Prepare a select statement
        $sql = "SELECT id, name, surname, password FROM users WHERE name = ? AND surname = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $name, $surname, $hashed_password);
...
<style>
  ...
  <body class="text-center footer-back-log">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="form-signin" method="post">
      ...
      <p>У вас нет аккаунта? <a href="register.php">Зарегистрировуйтесь</a>.</p>
      <br><br><br><br><br><br>
